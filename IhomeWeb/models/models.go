...
type Area struct {
	...
	Name   string   `orm:"size(32)" json:"aname"`       //区域名字    宝安   福田
	Houses []*House `orm:"reverse(many)" json:"houses"` //区域所有的房屋   与房屋表进行关联
}

/* 设施信息 table_name = "facility"*/     //设施信息 需要我们提前手动添加的
type Facility struct {
	Id     int      `json:"fid"`     //设施编号    1   2    3       4
	Name   string   `orm:"size(32)"` //设施名字    独卫 阳台  wifi	   空调
...
func init() {
	...
	orm.RegisterDataBase("default", "mysql", "root:1@tcp("+utils.G_mysql_addr+":"+utils.G_mysql_port+")/zgo3micro?charset=utf8", 30)
